...
class Resigner
	def resign(distribution_type, ipa_path, team_id=nil, app_id_prefix=nil)
		...
		Dir.chdir(tmpdir) do
			...
			zip(File.join(File.dirname(expended_ipa_path), "#{File.basename(expended_ipa_path, '.ipa')}-#{distribution_type}-resigned.ipa"))
		end
	...
	def add_swift_support_library()
		unless Dir["Payload/*.app/Frameworks/libswift*"].empty?
			...
			xcodebuild_version_string = `xcodebuild -version`
			match = xcodebuild_version_string.match(/Xcode ([\d|\.]*)/i)
			xcodebuild_version = match.captures.first

			swift_support_directory = 'SwiftSupport/iphoneos'
			if xcodebuild_version.to_f <= 7.2
				swift_support_directory = 'SwiftSupport'
			end
